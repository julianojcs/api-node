...
const locationsReqRules = {
    body: Joi.object().keys({
        ...
        uf: Joi.string().required().regex(/^[A-Z]{2}$/).messages({
            'string.base': `"uf" should be a type of 'text'`,
            'string.empty': `"uf" cannot be an empty field`,
            'string.pattern.base': `"uf" should have a 2 Letters in Upper Case`,
            'any.required': `"uf" is a required field`
          }),
        items: Joi.alternatives([Joi.array().items(Joi.number()), Joi.string()]) // https://joi.dev/api/?v=17.3.0#alternatives
        // items: Joi.array().items(Joi.number()).required() // .array().items(Joi.number()): fields are required and needs to be an array, and each items needs to be a number
    })
}
// const locationsReqRulesUpload = {
//     body: Joi.object().keys({
//         image: Joi.string().required().regex(/^([a-zA-Z0-9\s_\\.\\-\(\):])+(.gif|.jpg|.jpeg|.png)$/).default('fake-image.png')
//     })
// }
